...
public class SecurityFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (usuarioLogado == null) {
			((HttpServletResponse)response).sendRedirect("/Roupa/faces/login2.xhtml");
		} else {
			if (usuarioLogado.getPerfil().equals(Perfil.ADMINISTRADOR)
					|| usuarioLogado.getPerfil().equals(Perfil.CLIENTE)) {
				// permitindo a execucao comleta do protocolo
				chain.doFilter(request, response);
			} else {
				((HttpServletResponse)response).sendRedirect("/Roupa/faces/semacesso.xhtml");
			}
		}
